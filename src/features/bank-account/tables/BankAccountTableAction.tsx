...
import { useDeleteManyBankAccounts } from "../hooks";
import type { BankAccountResponse } from "../types";

type BankAccountTableActionBarProps = {
  table: Table<BankAccountResponse>;
};

export const BankAccountTableActionBar = ({
  table,
}: BankAccountTableActionBarProps) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const wasPendingRef = useRef(false);
  const rows = table.getFilteredSelectedRowModel().rows;
  const { onDelete, isPending } = useDeleteManyBankAccounts();
