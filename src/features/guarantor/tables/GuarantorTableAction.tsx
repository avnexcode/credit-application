...
import { useDeleteManyGuarantors } from "../hooks";
import type { GuarantorResponse } from "../types";

type GuarantorTableActionBarProps = {
  table: Table<GuarantorResponse>;
};

export const GuarantorTableActionBar = ({
  table,
}: GuarantorTableActionBarProps) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const wasPendingRef = useRef(false);
  const rows = table.getFilteredSelectedRowModel().rows;
  const { onDelete, isPending } = useDeleteManyGuarantors();
