...
type DashboardGuarantorPageProps = {
        ...
        >
          <header>
            <div className="flex w-full items-center justify-start gap-x-2 px-3">
              <Button asChild>
                <Link
                  href={`${env.NEXT_PUBLIC_BASE_URL}/dashboard/guarantor/create`}
                >
                  <Icon name="CirclePlus" /> Add Guarantor
                </Link>
              </Button>
              <TableSearch
...
DashboardGuarantorPage.getLayout = (page: React.ReactElement) => {
  const pageProps = page.props as DashboardGuarantorPageProps;
  return (
