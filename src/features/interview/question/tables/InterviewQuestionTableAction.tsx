...
import { useDeleteManyInterviewQuestions } from "../hooks";
import type { InterviewQuestionResponse } from "../types";

type InterviewQuestionTableActionBarProps = {
  table: Table<InterviewQuestionResponse>;
};

export const InterviewQuestionTableActionBar = ({
  table,
}: InterviewQuestionTableActionBarProps) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const wasPendingRef = useRef(false);
  const rows = table.getFilteredSelectedRowModel().rows;
  const { onDelete, isPending } = useDeleteManyInterviewQuestions();
