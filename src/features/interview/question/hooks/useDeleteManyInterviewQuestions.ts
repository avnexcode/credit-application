...
export const useDeleteManyInterviewQuestions = (): DeleteFunctionReturn<{
  ids: string[];
}> => {
  ...
  const {
    mutate: deleteInterviewQuestions,
    isPending: isDeleteInterviewQuestionsPending,
  } = api.interviewQuestion.deleteMany.useMutation({
    onSuccess: () => {
  ...
  const onDelete = (values: { ids: string[] }) => {
    deleteInterviewQuestions({ request: { ids: values.ids } });
  };

  return { onDelete, isPending: isDeleteInterviewQuestionsPending, defaultUrl };
};
