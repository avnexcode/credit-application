...
import { useDeleteManyCustomers } from "../hooks";
import type { CustomerResponse } from "../types";

type CustomerTableActionBarProps = {
  table: Table<CustomerResponse>;
};

export const CustomerTableActionBar = ({
  table,
}: CustomerTableActionBarProps) => {
  const { t } = useTranslation();
  const [isOpen, setIsOpen] = useState(false);
  const wasPendingRef = useRef(false);
  const rows = table.getFilteredSelectedRowModel().rows;
  const { onDelete, isPending } = useDeleteManyCustomers();
